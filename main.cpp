...
public:
    ...
    RayHit raycast(Vec3f origin, Vec3f direction, float maxDist) {
        ...
        if (direction.length() < 0.001f) return result;
        ...
        for (int i = 0; i < MAX_RAY_STEPS; i++) {
            ...
            BlockType block = getBlock(mapX, mapY, mapZ);
            if (block != BLOCK_AIR) {
                result.hit = true;
...
public:
        ...
        for (int bx = minX; bx <= maxX; bx++) {
            for (int by = minY; by <= maxY; by++) {
                for (int bz = minZ; bz <= maxZ; bz++) {
                    if (g_world.getBlock(bx, by, bz) != BLOCK_AIR) {
                        return true;
                    }
                }
            }
    ...
    bool breakBlock() {
        ...
        Vec3f dir = getForward();
        RayHit hit = g_world.raycast(eye, dir, 5.0f);
        if (hit.hit && hit.block != BLOCK_AIR) {
...
public:
    void init() {
        ...
        for (int screenY = 0; screenY < SCREEN_H; screenY += STEP) {
            for (int screenX = 0; screenX < SCREEN_W; screenX += STEP) {
                ...
                if (hit.hit) {
                    int side;
                    if (hit.normal.y > 0) side = 0;
                    else if (hit.normal.y < 0) side = 1;
                    else if (hit.normal.x != 0) side = 2;
                    else side = 3;
                ...
                M5Cardputer.Display.fillRect(screenX, screenY, STEP, STEP, color);
            }
        }
    }
    ...
    void drawHUD() {
        ...
        const char* blockNames[] = {"AIR", "STONE", "DIRT", "GRASS", "WOOD", "LEAVES"};
    ...
    bool handleMenuInput() {
        ...
        if (M5Cardputer.Keyboard.isChange()) {
            if (M5Cardputer.Keyboard.isPressed()) {
                ...
                if (left) {
                    currentWorldSlot--;
                    if (currentWorldSlot < 1) currentWorldSlot = 5;
                    checkSlot();
                    // 如果切换到了空槽，随机一个新种子
                ...
                if (right) {
                    currentWorldSlot++;
                    if (currentWorldSlot > 5) currentWorldSlot = 1;
                    checkSlot();
                    if (!slotHasSave) randomizeSeed();
                }
    ...
    for (size_t i = 0; i < keys.word.size(); i++) {
        ...
        switch (c) {
            ...
            case '/': case '.': 
                ...
                break;
            case '1': g_player.selectedBlock = BLOCK_DIRT; break;
            case '2': g_player.selectedBlock = BLOCK_STONE; break;
            case '3': g_player.selectedBlock = BLOCK_GRASS; break;
            case '4': g_player.selectedBlock = BLOCK_WOOD; break;
        }
    }
